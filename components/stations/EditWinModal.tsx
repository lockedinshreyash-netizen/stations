...
interface ImageSlot {
  preview: string; // object URL OR existing public URL
...
interface EditWinModalProps {
  win: WinCardData;
  onClose: () => void;
  onUpdated: (updated: Partial<WinCardData>) => void;
}
...
export default function EditWinModal({ win, onClose, onUpdated }: EditWinModalProps) {
  ...
  // Seed slots from existing images
  ...
  async function onSubmit(data: FormData) {
    if (images.some((s) => s?.uploading)) { setServerError("Images still uploading, please wait."); return; }
    ...
    >
      <div
        ...
        style={{
          ...
          border: "0.5px solid rgba(var(--fg-rgb),0.1)", borderRadius: 0,
        }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-6 sticky top-0"
          ...
          <button onClick={onClose} className="text-[rgba(var(--fg-rgb),0.4)] hover:text-[rgb(var(--fg-rgb))] transition-colors"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col px-8 py-6" style={{ gap: "22px" }}>
          ...
          <div className="flex flex-col" style={{ gap: "8px" }}>
            ...
            <input {...register("title")} type="text" className="font-poppins text-[rgb(var(--fg-rgb))] outline-none" style={fieldStyle} maxLength={80} />
          ...
          <div className="flex flex-col" style={{ gap: "8px" }}>
            ...
            <textarea {...register("description")} rows={5} className="font-poppins text-[rgb(var(--fg-rgb))] outline-none resize-none" style={{ ...fieldStyle, lineHeight: 1.7 }} maxLength={500} />
                ...
                return (
                  <div key={slotIndex} style={{ position: "relative", width: "120px", height: "120px", flexShrink: 0 }}>
                    {slot ? (
                      <>
                        {slot.uploading ? (
                          <div className="flex items-center justify-center" style={{ width: "120px", height: "120px", background: "var(--bg-surface)", border: "0.5px solid rgba(var(--fg-rgb),0.12)" }}>
                            <span className="font-poppins font-light text-[rgba(var(--fg-rgb),0.3)]" style={{ fontSize: "10px" }}>Uploading…</span>
                          </div>
                        ) : slot.error ? (
                          <div className="flex items-center justify-center" style={{ width: "120px", height: "120px", background: "var(--bg-surface)", border: "0.5px solid rgba(var(--accent-rgb),0.4)" }}>
                        ...
                        <button type="button" onClick={() => removeImage(slotIndex)} className="font-poppins"
                          style={{
                            position: "absolute", top: "4px", right: "4px", width: "20px", height: "20px",
                            background: "rgba(0,0,0,0.7)", color: "rgb(var(--fg-rgb))", border: "none", cursor: "pointer",
                            fontSize: "14px", lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 0,
                          }}>×</button>
                      </>
                    ) : (
                      <button type="button" onClick={() => fileRefs[slotIndex].current?.click()}
                        className="flex flex-col items-center justify-center transition-colors"
                        style={{ width: "120px", height: "120px", background: "transparent", border: "1px dashed rgba(var(--fg-rgb),0.2)", cursor: "pointer", gap: "6px", borderRadius: 0 }}>
          ...
          <div className="flex flex-col" style={{ gap: "8px" }}>
            ...
            <input {...register("media_url")} type="url" placeholder="Link to what you built…" className="font-poppins text-[rgb(var(--fg-rgb))] outline-none" style={fieldStyle} />
          ...
          <button type="submit" disabled={loading} className="font-poppins font-black uppercase text-[var(--bg-primary)]"
            style={{
              background: "rgb(var(--fg-rgb))", fontSize: "12px", letterSpacing: "0.15em", padding: "16px",
              border: "none", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1, transition: "opacity 150ms", borderRadius: 0,
            }}>
