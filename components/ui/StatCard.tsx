...
export default function StatCard({ label, value, unit }: StatCardProps) {
  return (
    <div className="st-card st-card-hover bg-[var(--bg-surface)] px-6 py-5 flex flex-col gap-1 min-w-0">
      <span className="text-[rgba(var(--fg-rgb),0.3)] text-sm tracking-widest uppercase font-light">
        {label}
      </span>
      <div className="flex items-baseline gap-1.5">
        ...
        {unit && (
          <span className="text-[rgba(var(--fg-rgb),0.3)] text-sm font-light">{unit}</span>
        )}
      </div>
    </div>
  );
